...
class PositionProfile(OperatingMode):
    ...
    def get_position_demand_value(self):
        """Output generated by the profiler and used as position control input

        Returns:
            position (int): current position output by the trajectory generator
        """
        return self._node.sdo[self.addresses.position_demand_value].raw

    def get_position_actual_value(self):
        """Actual position in user-defined units calculated using the position feedback

        Returns:
           position (int): encoder position in user-defined units
        """
        return self._node.sdo[self.addresses.position_actual_value].raw

    def get_position_actual_internal_value(self):
        """Actual position in internal units calculated using the position feedback

        Returns:
            position (int): encoder position in internal units
        """
        return self._node.sdo[self.addresses.position_actual_internal_value].raw

    def get_velocity_actual_value(self):
        """Actual velocity calculated using the velocity feedback.

        Returns:
            position/s (int): motor velocity in user-defined units
        """
